...
def get_alerts(username, password, app_name, env_id, start_time, end_time, limit=1000):
    ...
    filtered_alerts = []
    try:
        alerts = requests.get(url=mock_url, headers=headers, auth=(username, password), params=params).json()
        if type(alerts) == list:
            filtered_alerts = list(filter(lambda alert: start_time <= alert["createdAt"] <= end_time, alerts))
            logging.info("{} alerts found".format(len(filtered_alerts)))
...
def get_logs(username, password, env_id, start_time, end_time):
    ...
    domains_json = []
    try:
        domains_json = requests.get(url=mock_domain_url, headers=headers, data=body, auth=(username, password)).json()
    except Exception as err:
        logging.error("Something wrong. Exception error text: {}".format(err))
    logs = []
    if len(domains_json) == 0:
        ...
        if type(domains_json) is list:
            ...
            for domain in domains:
                ...
                try:
                    logs += requests.post(url=mock_log_url, headers=headers, auth=(username, password)).json()
                except Exception as err:
                    logging.error("Something wrong. Exception error text: {}".format(err))
                if any(type(log) == str for log in logs):
                    logs = []
                    logging.info("List of logs are empty")
            logging.info("{} logs found".format(len(logs)))
        else:
